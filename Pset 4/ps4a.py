    ...
    while (len(hand) > 0):
        ...
        if userinput == ".":
            ...
            if isValidWord(userinput, hand, wordList):
                handvalue = getWordScore(userinput, n)
                print('"' + userinput + '" ' + "earned " + str(handvalue) + " points")
                score += handvalue
                hand = updateHand(hand, userinput)
            else:
                print("Invalid word..")
    print("Game ended, the total score is: ", str(score))
...
def playGame(wordList):
    ...
    while (True):
        ...
        try:
            assert command in validcommands
            if command == 'e':
                break
            elif command == 'n':
